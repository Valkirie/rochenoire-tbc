...
void HostileRefManager::threatAssist(Unit* victim, float threat, SpellEntry const* threatSpell, bool singleTarget, bool ignoreTimer, bool isScaled, Unit *pSpellTarget)
{
    ...
    {
        validReference->getSource()->addThreat(victim, threatPerTarget, false, (threatSpell ? GetSpellSchoolMask(threatSpell) : SPELL_SCHOOL_MASK_NORMAL), threatSpell, true, isScaled, pSpellTarget);
        if (!ignoreTimer)
