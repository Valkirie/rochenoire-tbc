...
bool ChatHandler::HandleNpcChangeLevelCommand(char* args)
{
    ...
    if (lvl < 1 || lvl > sWorld.GetCurrentMaxLevel() + 3)
    {
...
bool ChatHandler::HandleLookupAccountIpCommand(char* args)
{
    ...
    //                                                 0            1         2        3        4
    QueryResult* result = LoginDatabase.PQuery("SELECT distinct a.id, username, ip, gmlevel, expansion FROM account a join account_logons b on(a.id=b.accountId) WHERE ip " _LIKE_ " " _CONCAT3_("'%%'", "'%s'", "'%%'"), ip.c_str());
