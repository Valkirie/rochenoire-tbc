...
float ObjectMgr::ScaleGold(uint32 in_level, uint32 out_level, uint32 gold, bool min) const
{
	uint32 expected_gold = min ? mingold_array[in_level - 1] : maxgold_array[in_level -1];
	float difference_gold = float((float)gold / (float)expected_gold);
	uint32 scaled_gold = min ? mingold_array[out_level - 1] : maxgold_array[out_level - 1];
	float scaled_gold_ratio = float((float)scaled_gold / (float)gold);
	gold *= scaled_gold_ratio;
