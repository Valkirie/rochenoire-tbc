...
static std::map<uint32, std::vector<float>> drop_map = {
...
void Player::CheckAreaExploreAndOutdoor()
{
    ...
    if (!(currFields & val))
    {
        ...
        else if (p->area_level > 0)
        {
            ...
            uint32 area = p->ID;
			uint32 area_level = p->area_level;
			if (const ZoneFlex* thisZone = sObjectMgr.getAreaZone(area))
				area_level = thisZone->LevelRangeMin;
            ...
            {
				int32 diff = int32(GetLevel()) - area_level;
                uint32 XP;
                if (diff < -5)
                {
                ...
                else if (diff > 5)
                {
                    ...
                    XP = uint32(sObjectMgr.GetBaseXP(area_level) * exploration_percent / 100 * sWorld.getConfig(CONFIG_FLOAT_RATE_XP_EXPLORE));
                }
                else
                {
                    XP = uint32(sObjectMgr.GetBaseXP(area_level) * sWorld.getConfig(CONFIG_FLOAT_RATE_XP_EXPLORE));
                }
...
void Player::SendPreparedGossip(WorldObject* pSource)
{
    ...
    m_playerMenu->SendGossipMenu(textId, pSource->GetObjectGuid(), this);
}
...
void Player::SendPreparedQuest(ObjectGuid guid) const
{
    ...
    if (QuestgiverGreeting const* data = sObjectMgr.GetQuestgiverGreetingData(guid.GetEntry(), type))
    {
        ...
        m_playerMenu->SendQuestGiverQuestList(qe, title, guid, this);
    }
    else
    {
        // single element case
        if (questMenu.MenuItemCount() == 1)
        {
            ...
            if (pQuest)
            {
                if (status == DIALOG_STATUS_REWARD_REP && !GetQuestRewardStatus(quest_id))
                    m_playerMenu->SendQuestGiverRequestItems(this, pQuest, guid, CanRewardQuest(pQuest, false), true);
                else if (status == DIALOG_STATUS_INCOMPLETE)
                    m_playerMenu->SendQuestGiverRequestItems(this, pQuest, guid, false, true);
                ...
                // TODO: verify if check for !pQuest->IsDailyOrWeekly() is really correct (possibly not)
                    ...
                    m_playerMenu->SendQuestGiverRequestItems(this, pQuest, guid, CanCompleteRepeatableQuest(pQuest), true);
                else
                    m_playerMenu->SendQuestGiverQuestDetails(this, pQuest, guid, true);
            }
        }
        // multiply entries
        else
...
bool Player::SatisfyQuestDay(Quest const* qInfo, bool msg) const
{
    ...
    for (uint32 quest_daily_idx = 0; quest_daily_idx < sWorld.GetCurrentMaxDaily(); ++quest_daily_idx)
    {
...
void Player::_LoadDailyQuestStatus(QueryResult* result)
{
    ...
    if (result)
    {
        ...
        do
        {
            if (quest_daily_idx >= sWorld.GetCurrentMaxDaily()) // max amount with exist data in query
            {
...
void Player::SaveToDB()
{
    ...
    SqlStatement uberInsert = CharacterDatabase.CreateStatement(insChar, "INSERT INTO characters (guid,account,name,race,class,gender,level,ilevel,xp,money,playerBytes,playerBytes2,playerFlags,"
