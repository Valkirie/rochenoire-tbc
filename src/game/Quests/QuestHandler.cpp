...
void WorldSession::HandleQuestgiverQueryQuestOpcode(WorldPacket& recv_data)
{
    ...
    if (Quest const* pQuest = sObjectMgr.GetQuestTemplate(quest))
        _player->GetPlayerMenu()->SendQuestGiverQuestDetails(_player, pQuest, pObject->GetObjectGuid(), true);
}
    ...
    if (pQuest)
    {
        if (_player->CanRewardQuest(pQuest, reward, true))
        {
            ...
            if (Quest const* nextquest = _player->GetNextQuest(guid, pQuest))
                _player->GetPlayerMenu()->SendQuestGiverQuestDetails(_player, nextquest, guid, true);
        }
        else
            _player->GetPlayerMenu()->SendQuestGiverOfferReward(_player, pQuest, guid, true);
    }
}
    ...
    if (Quest const* pQuest = sObjectMgr.GetQuestTemplate(quest))
    {
        if (_player->GetQuestStatus(quest) != QUEST_STATUS_COMPLETE)
        {
            if (pQuest->IsRepeatable())
                _player->GetPlayerMenu()->SendQuestGiverRequestItems(_player, pQuest, guid, _player->CanCompleteRepeatableQuest(pQuest), false);
            else
                _player->GetPlayerMenu()->SendQuestGiverRequestItems(_player, pQuest, guid, _player->CanRewardQuest(pQuest, false), false);
        }
        else
            _player->GetPlayerMenu()->SendQuestGiverRequestItems(_player, pQuest, guid, _player->CanRewardQuest(pQuest, false), false);
    }
    ...
    if (Quest const* pQuest = sObjectMgr.GetQuestTemplate(questId))
    {
        if (Group* pGroup = _player->GetGroup())
        {
            for (GroupReference* itr = pGroup->GetFirstMember(); itr != nullptr; itr = itr->next())
            {
                ...
                pPlayer->GetPlayerMenu()->SendQuestGiverQuestDetails(_player, pQuest, _player->GetObjectGuid(), true);
#endif
                pPlayer->SetDividerGuid(_player->GetObjectGuid());
