    ...
    void SpellHit(Unit* /*caster*/, const SpellEntry* spellEntry) override
    {
        if (spellEntry->Id == SPELL_WIDOWS_EMBRACE_1)
        {
            ...
            if (m_creature->HasAura(SPELL_ENRAGE))
            {
                ...
                ResetCombatAction(FAERLINA_ENRAGE, sObjectMgr.GetScaleSpellTimer(m_creature, 60 * IN_MILLISECONDS, SPELL_ENRAGE));   // Delay next enrage by 60 sec if Faerlina was already enraged
            }
        }
    }
        ...
        switch (action)
        {
            case FAERLINA_POISON_VOLLEY:
            {
                if (DoCastSpellIfCan(m_creature->GetVictim(), SPELL_POISONBOLT_VOLLEY) == CAST_OK)
                    ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, 11 * IN_MILLISECONDS, SPELL_POISONBOLT_VOLLEY));
                break;
            }
            case FAERLINA_RAIN_FIRE:
            {
                if (Unit* target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0))
                {
                    if (DoCastSpellIfCan(target, SPELL_RAIN_OF_FIRE) == CAST_OK)
                        ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, 16 * IN_MILLISECONDS, SPELL_RAIN_OF_FIRE));
                }
                break;
            }
