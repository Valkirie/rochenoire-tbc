...
struct boss_sapphironAI : public CombatAI
{
    ...
    uint32 GetSubsequentActionTimer(uint32 action)
    {
        switch (action)
        {
            case SAPPHIRON_CLEAVE: return sObjectMgr.GetScaleSpellTimer(m_creature, urand(5, 10) * IN_MILLISECONDS, SPELL_CLEAVE);
            case SAPPHIRON_TAIL_SWEEP: return sObjectMgr.GetScaleSpellTimer(m_creature, urand(7, 10) * IN_MILLISECONDS, SPELL_TAIL_SWEEP);
            case SAPPHIRON_LIFE_DRAIN: return sObjectMgr.GetScaleSpellTimer(m_creature, 24u * IN_MILLISECONDS, SPELL_LIFE_DRAIN);
            case SAPPHIRON_BLIZZARD: return sObjectMgr.GetScaleSpellTimer(m_creature, urand(10, 30) * IN_MILLISECONDS, SPELL_SUMMON_BLIZZARD_INIT);
            case SAPPHIRON_ICEBOLT: return sObjectMgr.GetScaleSpellTimer(m_creature, 3u * IN_MILLISECONDS, SPELL_ICEBOLT_INIT);
            case SAPPHIRON_BERSERK: return sObjectMgr.GetScaleSpellTimer(m_creature, 300u * IN_MILLISECONDS, SPELL_BESERK);
