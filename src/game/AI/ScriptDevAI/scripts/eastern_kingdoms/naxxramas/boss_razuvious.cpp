        ...
        switch (action)
        {
            case RAZUVIOUS_DISRUPTING_SHOUT:
            {
                if (DoCastSpellIfCan(m_creature, SPELL_DISRUPTING_SHOUT) == CAST_OK)
                    ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, 25u * IN_MILLISECONDS, SPELL_DISRUPTING_SHOUT));
                return;
            }
            case RAZUVIOUS_UNBALANCING_STRIKE:
            {
                if (DoCastSpellIfCan(m_creature->GetVictim(), SPELL_UNBALANCING_STRIKE) == CAST_OK)
                    ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, 30u * IN_MILLISECONDS, SPELL_UNBALANCING_STRIKE));
                return;
            }
        }
    }
