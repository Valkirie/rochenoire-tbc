...
struct boss_thaddiusAI : public CombatAI
{
    ...
    uint32 GetSubsequentActionTimer(uint32 id)
    {
        switch (id)
        {
            case THADDIUS_CHAIN_LIGHTNING: return sObjectMgr.GetScaleSpellTimer(m_creature, 15u * IN_MILLISECONDS, SPELL_CHAIN_LIGHTNING);
            case THADDIUS_POLARITY_SHIFT: return sObjectMgr.GetScaleSpellTimer(m_creature, 30u * IN_MILLISECONDS, SPELL_POLARITY_SHIFT);
            case THADDIUS_BALL_LIGHTNING: return sObjectMgr.GetScaleSpellTimer(m_creature, 3u * IN_MILLISECONDS, SPELL_BALL_LIGHTNING);
            case THADDIUS_BERSERK: return sObjectMgr.GetScaleSpellTimer(m_creature, 10u * MINUTE * IN_MILLISECONDS, SPELL_BESERK);
            default: return 0;
        }
    }
