        ...
        switch (action)
        {
            ...
            case PATCHWERK_HATEFUL_STRIKE:
            {
                if (DoCastSpellIfCan(m_creature, SPELL_HATEFULSTRIKE_PRIMER) == CAST_OK)
                    ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, 1.2 * IN_MILLISECONDS, SPELL_HATEFULSTRIKE_PRIMER));
                break;
            }
            ...
            case PATCHWERK_BERSERK_SILMEBOLT:
            {
                if (DoCastSpellIfCan(m_creature->GetVictim(), SPELL_SLIMEBOLT) == CAST_OK)
                    ResetCombatAction(PATCHWERK_BERSERK_SILMEBOLT, sObjectMgr.GetScaleSpellTimer(m_creature, 1 * IN_MILLISECONDS, SPELL_SLIMEBOLT));
                break;
            }
            default:
                break;
