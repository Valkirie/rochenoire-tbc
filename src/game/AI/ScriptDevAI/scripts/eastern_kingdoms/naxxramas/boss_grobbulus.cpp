...
struct boss_grobbulusAI : public CombatAI
{
    ...
    uint32 GetSubsequentActionTimer(uint32 id)
    {
        switch (id)
        {
            ...
            case GROBBULUS_INJECTION: return sObjectMgr.GetScaleSpellTimer(m_creature, m_creature->GetHealthPercent() > 30.0f ? urand(7, 13) * IN_MILLISECONDS : urand(3, 7) * IN_MILLISECONDS, SPELL_MUTATING_INJECTION);
            case GROBBULUS_POISON_CLOUD: return sObjectMgr.GetScaleSpellTimer(m_creature, 15u * IN_MILLISECONDS, SPELL_POISON_CLOUD);
            case GROBBULUS_SLIME_SPRAY: return sObjectMgr.GetScaleSpellTimer(m_creature, urand(20, 30) * IN_MILLISECONDS, SPELL_SLIME_SPRAY);
            default: return 0;
        }
    }
    ...
    void ExecuteAction(uint32 action) override
    {
        switch (action)
        {
            case GROBBULUS_SLIME_STREAM:
            {
                ...
                ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, 3 * IN_MILLISECONDS, SPELL_SLIME_STREAM));
                break;
            }
