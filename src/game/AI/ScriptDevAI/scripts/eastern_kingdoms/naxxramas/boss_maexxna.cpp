        ...
        switch (action)
        {
            ...
            case MAEXXNA_POISON_SHOCK:
            {
                if (DoCastSpellIfCan(m_creature->GetVictim(), SPELL_POISONSHOCK) == CAST_OK)
                    ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, urand(10, 20) * IN_MILLISECONDS, SPELL_POISONSHOCK));
                break;
            }
            case MAEXXNA_NECROTIC_POISON:
            {
                if (DoCastSpellIfCan(m_creature->GetVictim(), SPELL_NECROTICPOISON) == CAST_OK)
                    ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, urand(20, 30) * IN_MILLISECONDS, SPELL_NECROTICPOISON));
                break;
            }
