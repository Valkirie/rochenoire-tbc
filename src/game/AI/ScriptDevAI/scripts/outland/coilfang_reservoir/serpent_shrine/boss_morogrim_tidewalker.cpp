        ...
        switch (action)
        {
            ...
            case MOROGRIM_TIDAL_WAVE:
            {
                if (DoCastSpellIfCan(nullptr, SPELL_TIDAL_WAVE) == CAST_OK)
                    ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, urand(20000, 25000), SPELL_TIDAL_WAVE));
                break;
            }
            case MOROGRIM_WATERY_GRAVE:
            {
                if (DoCastSpellIfCan(nullptr, SPELL_WATERY_GRAVE) == CAST_OK)
                {
                    ...
                    ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, 30000, SPELL_WATERY_GRAVE));
                }
                break;
            }
