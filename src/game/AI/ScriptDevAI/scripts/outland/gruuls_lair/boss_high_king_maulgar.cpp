            ...
            case MAULGAR_PHASE_2:
                if (m_creature->GetHealthPercent() < 50.0f)
                {
                    if (DoCastSpellIfCan(nullptr, SPELL_FLURRY) == CAST_OK)
                    {
                        ...
                        ResetCombatAction(MAULGAR_FEAR, sObjectMgr.GetScaleSpellTimer(m_creature, urand(10000, 25000), SPELL_FLURRY));
                        ResetCombatAction(MAULGAR_CHARGE, sObjectMgr.GetScaleSpellTimer(m_creature, 2000, SPELL_FLURRY));
                    }
                }
                break;
            case MAULGAR_ARCING_SMASH:
                if (DoCastSpellIfCan(nullptr, SPELL_ARCING_SMASH) == CAST_OK)
                    ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, urand(8000, 12000), SPELL_ARCING_SMASH));
                break;
            case MAULGAR_MIGHTY_BLOW:
                if (DoCastSpellIfCan(m_creature->GetVictim(), SPELL_MIGHTY_BLOW) == CAST_OK)
                    ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, urand(20000, 35000), SPELL_MIGHTY_BLOW));
                break;
            case MAULGAR_WHIRLWIND:
                if (DoCastSpellIfCan(nullptr, SPELL_WHIRLWIND) == CAST_OK)
                    ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, urand(30000, 40000), SPELL_WHIRLWIND));
                break;
            case MAULGAR_CHARGE:
                if (Unit* target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 1, SPELL_CHARGE, SELECT_FLAG_PLAYER))
                    if (DoCastSpellIfCan(target, SPELL_CHARGE) == CAST_OK)
                        ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, urand(14000, 20000), SPELL_CHARGE));
                break;
            case MAULGAR_FEAR:
                if (DoCastSpellIfCan(m_creature->GetVictim(), SPELL_FEAR) == CAST_OK)
                    ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, urand(20000, 35000), SPELL_FEAR));
                break;
        }
    }
};
        ...
        switch (action)
        {
            case OLM_DARK_DECAY:
                if (DoCastSpellIfCan(m_creature->GetVictim(), SPELL_DARK_DECAY) == CAST_OK)
                    ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, 5000, SPELL_DARK_DECAY));
                break;
            case OLM_DEATH_COIL:
                if (DoCastSpellIfCan(m_creature->GetVictim(), SPELL_DEATH_COIL) == CAST_OK)
                    ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, urand(8000, 13000), SPELL_DEATH_COIL));
                break;
            case OLM_SUMMON:
                if (DoCastSpellIfCan(nullptr, SPELL_SUMMON_WILD_FELHUNTER) == CAST_OK)
                    ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, urand(25000, 35000), SPELL_SUMMON_WILD_FELHUNTER));
                break;
        }
    }
};
        ...
        switch (action)
        {
            case KIGGLER_ACTION_GREATER_POLYMORPH:
                if (DoCastSpellIfCan(m_creature->GetVictim(), SPELL_GREATER_POLYMORPH) == CAST_OK)
                    ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, 11000, SPELL_GREATER_POLYMORPH));
                return;
            case KIGGLER_ACTION_ARCANE_EXPLOSION:
                if (m_creature->SelectAttackingTarget(ATTACKING_TARGET_NEAREST_BY, 0, SPELL_ARCANE_EXPLOSION, SELECT_FLAG_PLAYER | SELECT_FLAG_USE_EFFECT_RADIUS))
                    if (DoCastSpellIfCan(nullptr, SPELL_ARCANE_EXPLOSION) == CAST_OK)
                        ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, 30000, SPELL_ARCANE_EXPLOSION));
                return;
            case KIGGLER_ACTION_ARCANE_SHOCK:
                if (DoCastSpellIfCan(m_creature->GetVictim(), SPELL_ARCANE_SHOCK) == CAST_OK)
                    ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, urand(15000, 20000), SPELL_ARCANE_SHOCK));
                return;
            case KIGGLER_ACTION_LIGHTNING_BOLT:
                if (DoCastSpellIfCan(m_creature->GetVictim(), SPELL_LIGHTNING_BOLT) == CAST_OK)
                    ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, GetCurrentRangedMode() ? urand(2000, 3000) : urand(5000, 8000), SPELL_LIGHTNING_BOLT));
            break;
        }
    }
};
        ...
        switch (action)
        {
            case BLINDEYE_GREATER_POWER_WORD_SHIELD:
                if (DoCastSpellIfCan(nullptr, SPELL_GREATER_PW_SHIELD) == CAST_OK)
                {
                    ...
                    ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, urand(30000, 40000), SPELL_PRAYEROFHEALING));
                }
                break;
            case BLINDEYE_HEAL:
                if (Unit* target = DoSelectLowestHpFriendly(50.0f))
                    if (DoCastSpellIfCan(target, SPELL_HEAL) == CAST_OK)
                        ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, urand(15000, 40000), SPELL_HEAL));
                break;
        }
    }
};
            ...
            case KROSH_BLAST_WAVE:
                if (m_creature->SelectAttackingTarget(ATTACKING_TARGET_NEAREST_BY, 0, nullptr, SELECT_FLAG_PLAYER | SELECT_FLAG_USE_EFFECT_RADIUS))
                    if (DoCastSpellIfCan(nullptr, SPELL_BLAST_WAVE) == CAST_OK)
                        ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, 6000, SPELL_BLAST_WAVE));
                break;
            case KROSH_SPELL_SHIELD:
                if (DoCastSpellIfCan(nullptr, SPELL_SPELLSHIELD) == CAST_OK)
                    ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, 30000, SPELL_SPELLSHIELD));
                break;
            case KROSH_GREATER_FIREBALL:
                if (DoCastSpellIfCan(m_creature->GetVictim(), SPELL_GREATER_FIREBALL) == CAST_OK)
                    ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, 3200, SPELL_GREATER_FIREBALL));
                break;
        }
    }
};
