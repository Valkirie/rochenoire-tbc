...
struct boss_viscidusAI : public CombatAI
{
    ...
    void Reset() override
    {
        ...
        m_uiHitCount 			= 0;
        m_uiHitCount_Slow 		= 0;
        m_uiHitCount_SlowMore 	= 0;
        m_uiHitCount_Freeze 	= 0;
        m_uiHitCount_Crack 		= 0;
        m_uiHitCount_Shatter 	= 0;
        m_uiHitCount_Explode 	= 0;
    ...
    void UpdateHitCounts()
    {
        ...
        m_uiHitCount_Explode = ((DungeonMap*)m_creature->GetMap())->GetFinalNAdds(m_creature->GetInstanceTanks(), HITCOUNT_EXPLODE);
    }
    ...
    void IncreaseHitCount()
    {
        ...
        switch (m_phase)
        {
            case PHASE_NORMAL:
            {
                if (m_hitCount == m_uiHitCount_Slow)
                    SetPhase(PHASE_SLOWED);
                break;
            }
            case PHASE_SLOWED:
            {
                if (m_hitCount == m_uiHitCount_SlowMore)
                    SetPhase(PHASE_SLOWED_MORE);
                break;
            }
            case PHASE_SLOWED_MORE:
            {
                if (m_hitCount == m_uiHitCount_Freeze)
                    SetPhase(PHASE_FROZEN);
                break;
            }
            case PHASE_FROZEN:
            {
                if (m_hitCount == m_uiHitCount_Crack)
                    DoScriptText(EMOTE_CRACK, m_creature);
                else if (m_hitCount == m_uiHitCount_Shatter)
                    DoScriptText(EMOTE_SHATTER, m_creature);
                else if (m_hitCount == m_uiHitCount_Explode)
