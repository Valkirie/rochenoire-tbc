...
struct boss_fankrissAI : public CombatAI
{
    ...
    void ExecuteAction(uint32 action) override
    {
        switch (action)
        {
            ...
            {
                uint32 spellId = m_summonWormSpells[m_wormsSpawnPerWave - 1];
                if (DoCastSpellIfCan(nullptr, spellId) == CAST_OK)
                {
                    ...
                    if (!m_wormsSpawnPerWave)
                    {
                        ...
                        ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, urand(5000, 7000), spellId));
                }
                break;
            }
            case FANKRISS_ENTANGLE:
            {
                ...
                if (Unit* target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, uint32(0), SELECT_FLAG_PLAYER | SELECT_FLAG_SKIP_TANK))
                {
                    ...
                    if (DoCastSpellIfCan(target, spellId) == CAST_OK)
                    {
                        ...
                        ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, urand(25000, 55000), spellId));
                    }
                }
