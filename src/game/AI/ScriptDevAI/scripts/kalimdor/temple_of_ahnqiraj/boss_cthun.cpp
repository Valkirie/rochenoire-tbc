            ...
            case EYECTHUN_EYEBEAM:
                ...
                if (m_eyeBeamCount < 3)
                {
                    if (m_creature->GetVictim())
                    {
                        ...
                        {
                            ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, urand(2, 3) * IN_MILLISECONDS, SPELL_EYE_BEAM_INIT));
                            ++m_eyeBeamCount;
                        }
                    }
                }
                ...
                else if (Unit* target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0))
                {
                    if (DoCastSpellIfCan(target, SPELL_EYE_BEAM) == CAST_OK)
                        ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, urand(2, 3) * IN_MILLISECONDS, SPELL_EYE_BEAM));
                }
                break;
...
struct boss_cthunAI : public CombatAI
{
    ...
    void SummonedCreatureJustDied(Creature* summoned) override
    {
        ...
        if (summoned->GetEntry() == NPC_FLESH_TENTACLE)
        {
            ...
            if (m_fleshTentaclesKilled == m_fleshTentaclesSpawned)
                HandleWeaken();
        }
    }
    ...
    void DoSpawnTentacles()
    {
        ...
        m_fleshTentaclesKilled = 0;
		m_fleshTentaclesSpawned = ((DungeonMap*)m_creature->GetMap())->GetFinalNAdds(m_creature->GetInstanceTanks(), MAX_FLESH_TENTACLES);
        for (uint8 i = 0; i < m_fleshTentaclesSpawned; ++i)
        ...
        switch (action)
        {
            case GIANTCLAW_HAMSTRING:
                if (DoCastSpellIfCan(m_creature->GetVictim(), SPELL_HAMSTRING) == CAST_OK)
                    ResetCombatAction(action, sObjectMgr.GetScaleSpellTimer(m_creature, 10 * IN_MILLISECONDS, SPELL_HAMSTRING));
                break;
